...
class Midjourney:
    ...
    async def generate(self, prompt: str, n: int, *, width: WH, height: WH, publish: bool = True) -> ReplicateResult:
        ...
        if width + height == (1024 + 768) and n > 1:
            ...
            for i in range(n - 1):
                imgs.append(
                    await self.replicate.run(self.MODEL_VERSION, wait=True, prompt=prompt, width=width, height=height)
                )
            ...
            for i in range(math.ceil((n - 1) / 4)):
                _tasks.append(
                    self.replicate.run(
                        self.MODEL_VERSION, wait=True, prompt=prompt, width=width, height=height, num_outputs=4
                    )
                )
