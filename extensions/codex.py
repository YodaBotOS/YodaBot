...
class Codex(commands.Cog):
    ...
    async def generate_code(self, ctx: Context, language: core_codex.SUPPORTED_LANGUAGES_LITERAL, *, prompt: str):
        ...
        async with ctx.typing():
            ...
            if len(completion) > 3000:
                paste = await self.bot.mystbin.create_paste(filename=f'code.{self.codex.FILE[language]}', content=completion)
    ...
    @commands.max_concurrency(1, commands.BucketType.user)
    @commands.cooldown(1, 20, commands.BucketType.user)
    async def explain_code(self, ctx: Context, language: Optional[core_codex.SUPPORTED_LANGUAGES_LITERAL] = None, *, code: CodeblockConverter):
        """
